...
// 2026-07-23 food_top='' (분류기가 한 번 보고 못 알아본) 7/13~7/26 데이터를 다시 훑어서 실제로
// 놓친 식품(비에날씬 계열, 순대국, 소한마리탕)만 추가함 — 나머지 미분류 대다수는 옷/가전/보험/
// 주얼리/화장품처럼 애초에 식품이 아닌 상품이라 분류기가 null을 반환하는 게 정상 동작이었다.
...
const SUPPLEMENT_DEFS = [
  { key: 'probiotics', label: '유산균 · 프로바이오틱스', re: /유산균|프로바이오틱스|BNR17|덴티백|당큐락|메노락토|테라바이오틱스|오라틱스|비에날씬|비에날퀸/ },
  { key: 'omega3', label: '오메가3', re: /오메가/ },
  { key: 'chondroitin', label: '콘드로이친', re: /콘드\s?로이친/ },
  { key: 'mbp', label: 'MBP', re: /MBP|엠비피/ },
  { key: 'nem', label: 'NEM', re: /NEM|엔이엠|DEM|디이엠/ },
  { key: 'anapalactin', label: '아나파랙틴', re: /아나파랙틴/ },
  { key: 'eye', label: '루테인 · 지아잔틴 · 아스타잔틴', re: /루테인|지아잔틴|아스타잔틴|아스타\s?루지/ },
  ...
  { key: 'ginger', label: '생강진액 · 청', re: /생강진액|생강청/ },
  ...
  { key: 'protein', label: '프로틴', re: /프로틴|하이뮨|산양유/ },
  ...
  { key: 'kamut_faro', label: '카무트 · 파로 효소', re: /카무트|그레인온|파로\s?효소|골드파로|그라노토스칸|그라노벨로|파로면|파로\s?저당|파로\s?고단백|파로누룽지/ },
  { key: 'hempseed', label: '대마종자유', re: /대마종자유/ },
  { key: 'lemon_juice', label: '레몬즙', re: /레몬즙/ },
  { key: 'berry_juice', label: '블루베리 · 베리류 즙', re: /블루베리|빌베리|하스카프베리|그랑베리|아사이베리/ },
  { key: 'yam', label: '참마 분말', re: /참마/ },
  { key: 'raspberry', label: '복분자', re: /복분자/ },
  { key: 'rawgrain_meal', label: '생식 · 라이프밀', re: /생식|라이프밀/ },
  { key: 'bittermelon', label: '여주 발효효소', re: /여주(?!쌀)/ },
  { key: 'kudzu', label: '칡즙', re: /칡즙/ },
  { key: 'tomato_juice', label: '토마토 착즙', re: /토마토(?!케찹)/ },
  { key: 'melatonin', label: '멜라토닌', re: /멜라토닌|멜라바인/ },
  { key: 'olive', label: '올리브', re: /올리브/ },
  { key: 'wine_vinegar', label: '와인비니거', re: /비니거/ },
  { key: 'acerola', label: '아세로라', re: /아세로라/ },
  { key: 'glucose', label: '혈당관리', re: /혈당|당케어|당플랜/ },
  { key: 'mastic', label: '위엔 매스틱', re: /위엔\s?매스틱/ },
  { key: 'biotin', label: '비오틴 · 맥주효모', re: /비오틴|맥주효모/ },
  { key: 'goji', label: '구기자', re: /구기자/ },
  { key: 'menopause_etc', label: '갱년기', re: /갱년기/ },
  { key: 'garlic', label: '흑마늘 · 자연한알', re: /흑마늘|자연한알|마늘한알/ },
];
...
const FOOD_DEFS = [
  ...
  { key: 'croaker', label: '민어', re: /민어/ },
  { key: 'seabream', label: '참돔', re: /참돔/ },
  { key: 'pomfret', label: '병어', re: /병어/ },
  { key: 'tilefish', label: '옥돔', re: /옥돔/ },
  { key: 'rockfish', label: '볼락', re: /볼락/ },
  { key: 'seabass', label: '농어', re: /농어/ },
  // 축산물 — 구체적 요리명을 원재료명보다 먼저 체크
  { key: 'gamjatang', label: '감자탕', re: /감자탕/ },
  ...
  { key: 'sundaeguk', label: '순대국', re: /순대국/ },
  { key: 'sohanmaritang', label: '소한마리탕', re: /소한마리탕/ },
  { key: 'naengmyeon', label: '냉면', re: /냉면|랭면/ },
  { key: 'gopchang', label: '곱창전골', re: /곱창전골/ },
  ...
  { key: 'la_galbi', label: 'LA갈비 · 갈비찜', re: /LA갈비|갈비찜/ },
  { key: 'polib', label: '폭립', re: /폭립/ },
  { key: 'smoked_chicken', label: '훈제치킨', re: /훈제치킨/ },
  { key: 'duck_roast', label: '오리로스', re: /오리로스/ },
  { key: 'jerky', label: '육포', re: /육포/ },
  { key: 'boyangtang', label: '보양탕 · 수육탕', re: /보양탕|수육탕/ },
  { key: 'chicken_thigh', label: '닭다리살', re: /닭다리살/ },
  { key: 'brisket', label: '차돌박이', re: /차돌박이/ },
  { key: 'makchang', label: '막창', re: /막창/ },
  ...
  { key: 'corn', label: '찰옥수수', re: /찰\s?옥수수/ },
  { key: 'chili_powder', label: '고춧가루', re: /고춧가루/ },
  { key: 'potato', label: '감자', re: /감자(?!탕)/ },
  { key: 'onion', label: '양파', re: /양파/ },
  { key: 'sweet_potato', label: '고구마', re: /고구마/ },
  { key: 'sweet_pumpkin', label: '단호박', re: /단호박|밤호박/ },
  ...
  { key: 'jangajji', label: '장아찌 · 오이지', re: /장아찌|오이지/ },
  // 유제품 · 음료
  { key: 'soymilk', label: '두유 · 콩물', re: /두유|콩물/ },
  { key: 'juice', label: '주스', re: /주스|쥬스/ },
  { key: 'yogurt', label: '요거트', re: /요거트/ },
  // 기타 가공식품
  { key: 'sauce', label: '장류 · 소스', re: /간장소스|만능소스|양념장/ },
  { key: 'noodle', label: '막국수 · 메밀면', re: /막국수|메밀면/ },
  { key: 'ramyeon', label: '라면 · 짜장면', re: /라면|짜장/ },
  { key: 'konjac_noodle', label: '두부면 · 우무면', re: /두부면|우무면|곤약면/ },
  { key: 'mandu', label: '만두', re: /만두/ },
  { key: 'tteok', label: '떡 · 송편', re: /떡|송편/ },
  { key: 'sesame_oil', label: '참기름', re: /참기름/ },
  { key: 'jeonbyeong', label: '전병', re: /전병/ },
  { key: 'granola', label: '그래놀라', re: /그래놀라/ },
  { key: 'nuts', label: '견과', re: /견과/ },
  { key: 'seaweed', label: '김', re: /재래김|파래김|도시락김|구운김|조미김/ },
  { key: 'omija', label: '오미자', re: /오미자/ },
  { key: 'broth', label: '육수', re: /육수/ },
  { key: 'coffee', label: '커피', re: /커피(?!머신)/ },
  { key: 'hummus', label: '후무스', re: /후무스/ },
  { key: 'yanggaeng', label: '양갱', re: /양갱/ },
];
