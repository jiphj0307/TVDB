...
- Fresh Season: 제철 먹거리·건강·TV레시피를 다루는 별도의 블로그 프로젝트. 지금 이 요청은 "쓸 수 있는지 검토·확인"이 아니라, Fresh Season 블로그 글을 실제로 작성할 때 TVDB에 모아둔 이 캡처 이미지들을 그 안에 활용하라는 것이다.
...
절차:
1. 연결된 MCP 서버 설명 중 "TVDB"·"홈쇼핑/방송편성표"가 언급된 서버를 찾아, 그 서버의 run_sql 도구로 "SELECT * FROM tvdb_program_episodes WHERE id=${ep.id}"를 실행해 그 행을 조회할 것 — get_rows는 filter 조건이 무시되고 최신순 상위 목록만 반환되는 경우가 같은 계열 MCP 서버에서 확인된 적이 있어, 특정 id 하나를 정확히 찾을 땐 get_rows보다 run_sql이 안전하다
