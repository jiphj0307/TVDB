...
import ShoppingFoodView from './ShoppingFoodView';
...
export default function ShoppingPanel({ showToast }) {
  ...
  const [viewMode, setViewMode] = useState('channel'); // 'channel' | 'food'

  useEffect(() => { loadChannels(); }, []);
  useEffect(() => { if (activeChannel) loadChannelData(activeChannel); }, [activeChannel]);

  // 채널/날짜/보기모드 선택 -> URL 쿼리스트링 동기화 (새로고침해도 유지됨)
  useEffect(() => {
  ...
  }, [activeChannel, selectedDate, viewMode]);

  useEffect(() => {
    if (!router.isReady) return;
    const q = router.query;
    if (q.tab === 'shopping' && (q.view === 'channel' || q.view === 'food')) setViewMode(q.view);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [router.isReady]);
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 16 }}>
        ...
        <button onClick={() => setViewMode('food')} style={{
          padding: '8px 16px', borderRadius: 8, border: '1px solid #d1e8d1',
          background: viewMode === 'food' ? '#16a34a' : '#fff',
          color: viewMode === 'food' ? '#fff' : '#4b6e4b',
          fontWeight: 700, fontSize: 13, cursor: 'pointer', fontFamily: "'Outfit', sans-serif",
        }}>🍑 과일 · 건기식 · 식품 정리</button>
      </div>

      {viewMode === 'food' && <ShoppingFoodView />}
